<x-app-layout>
    ...
    <div class="max-w-2xl">
        <div class="bg-white shadow-sm rounded-xl p-6 space-y-4">
            <div>
                <span class="text-xs text-gray-400 uppercase tracking-wider">Naam</span>
                <p class="text-gray-800 font-medium">{{ $product->name }}</p>
            </div>
            <div>
                <span class="text-xs text-gray-400 uppercase tracking-wider">Categorie</span>
            ...
            <div class="pt-4 flex gap-3">
                <a href="{{ route('product.edit', $product->id) }}" class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-5 py-2 rounded-lg transition">Bewerken</a>
                <a href="{{ route('product.index') }}" class="text-sm text-gray-500 hover:underline px-3 py-2">Terug</a>
            </div>
        </div>
    </div>
</x-app-layout>
